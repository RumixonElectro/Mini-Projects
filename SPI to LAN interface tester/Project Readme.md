# **W5500 Automated Production Tester Jig (v4.0)**

A high-speed, sequential hardware validation fixture designed to rapidly diagnose **W5500 Ethernet Micro Modules**.

The jig executes hard-coded hardware and network diagnostic routines under a unified user-configurable timeout threshold (![][image1] default) and translates failures into distinct audio-visual signals for immediate line operator identification.

## **🔌 Hardware Wiring Diagram**

Ensure your test jig is wired strictly to the following pinout configuration. The buzzer and LED indicators require no external driver boards and can run directly from the digital pins (using standard current-limiting resistors).

| Peripheral / Module | Pin (W5500 Board) | Target Arduino Pin | Description |
| :---- | :---- | :---- | :---- |
| **W5500 Transceiver** | VCC | 3V3 | Power Supply (![][image2]) |
|  | GND | GND | Common System Ground |
|  | SCS / CS | D10 | SPI Chip Select |
|  | MOSI | D11 | SPI Master Out Slave In |
|  | MISO | D12 | SPI Master In Slave Out |
|  | SCLK | D13 | SPI Clock |
|  | RST | D3 | Hardware Reset Control |
| **Buzzer** | \+ (Anode) | D4 | Active High Piezo Buzzer |
| **Green LED** | \+ (Anode) | D2 | Success Indicator LED |

## **🚦 Diagnostic Workflow**

On boot or hardware reset, the firmware executes diagnostic checks in a **strict, non-blocking sequential pipeline**:

\[ Power ON \] ──\> \[ Boot Chirp \] ──\> \[ Hardware Reset \]   
                                           │  
                                           ▼  
\[ Halt (Loop) \] \<── \[ Fail: 5 Beeps \] \<── \[ Phase 1: SPI Check \]  
                                           │ (Success)  
                                           ▼  
\[ Halt (Loop) \] \<── \[ Fail: 2 Beeps \] \<── \[ Phase 2: DHCP Check \]  
                                           │ (Success)  
                                           ▼  
                                    \[ Success State \]   
                                    (Green LED \+ LCD IP)  
                                           │  
                                           ▼  
                                    \[ Live Mesh Mode \]   
                                    (UDP Listening)

### **1\. Initial Power Check**

* **Sensory Action:** Buzzer sounds instantly for exactly ![][image3].  
* **LCD Display:** V4 Jig Booting...  
* **Behind the Scenes:** Pulls physical pin D3 LOW for ![][image4] and releases it to HIGH for ![][image5] to cleanly reboot the W5500 ASIC.

### **2\. Phase 1: SPI Hardware Check**

* **Timeout Window:** Defined by the global TIMEOUT\_MS variable (![][image1]).  
* **Logic:** Continuously polls the W5500 SPI registers to verify connection.  
* **On Failure:** If no hardware is detected before the timeout:  
  * LCD displays: SPI: FAILED / CHIP NOT FOUND.  
  * Buzzer sounds exactly **5 times** (![][image3] beep / ![][image3] gap) and halts.

### **3\. Phase 2: DHCP Network Allocation Check**

* **Timeout Window:** Defined by the global TIMEOUT\_MS variable (![][image1]).  
* **Logic:** Requests a dynamic local IP lease using a randomized MAC address to prevent network address collisions.  
* **On Failure:** If the lease request times out:  
  * LCD displays: DHCP: FAILED / NO IP ASSIGNED.  
  * Buzzer sounds exactly **2 times** (![][image3] beep / ![][image3] gap) and halts.

### **4\. Initialization Validation (Success State)**

* **Sensory Action:** Green OK LED (pin D2) switches **HIGH**.  
* **Buzzer Action:** Fires a double-confirmation beep (![][image3] beep / ![][image3] gap).  
* **LCD Display:** Displays the leased Local IP Address on line 1, and Module is OK on line 2\.

### **5\. Runtime Passive Mesh Monitor**

* **Port / Protocol:** UDP Port 8888\.  
* **Action:** Listens for broadcast network test packets. If a packet starts with the token signature TG\_NODE:, the LCD displays the payload, the buzzer chirps once (![][image3] alert), and the interface automatically restores to idle state after ![][image6].

## **📂 Code Architecture**

The firmware (W5500\_Tester\_Jig\_v4.ino) is constructed to avoid background delays and race conditions:

* **generateRandomMAC()**: Utilizes floating analog noise from pins A0 and A1 to formulate a unique local MAC address on every power cycle.  
* **setup()**: Handles the sequential check blocks. It forces an initial static configuration (dummyIP) to awaken the Arduino SPI bus registers before calling Ethernet.hardwareStatus(), resolving library incompatibilities.  
* **loop()**: Operates purely as a passive UDP mesh network packet processor, resetting the LCD automatically after successful packets to keep the test environment ready.  
* **TIMEOUT\_MS Global Configuration**: Easily adjust testing speeds at the top of the file:  
  const unsigned long TIMEOUT\_MS \= 2000; // Change this to speed up/slow down testing cycles  


[image1]: <data:image/png;base64,>

[image2]: <data:image/png;base64,>

[image3]: <data:image/png;base64,>

[image4]: <data:image/png;base64,>

[image5]: <data:image/png;base64,>

[image6]: <data:image/png;base64,>